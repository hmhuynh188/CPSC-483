...
# employment rate and income graph 
...
# employment rate and NGDP - error 
...
weo_raw = pd.read_csv('/content/drive/My Drive/Colab Notebooks/WEOOct2023all.csv')
na_mask = weo_raw['WEO Subject Code'].isna()
weo_selected_measurement = weo_raw.loc[~na_mask & weo_raw['WEO Subject Code'].str.contains("NGDP")]
weo_selected_measurement_2018 = pd.DataFrame(weo_selected_measurement, columns=['Country', '2018'])
print(weo_selected_measurement_2018)

merged_train_data = pd.merge(hls_train, weo_selected_measurement_2018, on="Country")
merged_train_data = merged_train_data.rename(columns={"Value": "Employment Rate", "2018": "Total Nominal Income"})
merged_train_data = pd.DataFrame(merged_train_data, columns=['Country','Employment Rate', 'Total Nominal Income'])
...
X = np.c_[merged_train_data["Total Nominal Income"]]
Y = np.c_[merged_train_data["Employment Rate"]]
x = X.tolist()
y = Y.tolist()
